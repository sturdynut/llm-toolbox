...
Two Claude Code hooks and a status line that measure what a session costs: `context-guard`
(context fill, loop detection, semantic drift) and `session-weight` (the static baseline
every session pays before you type).

**Status: pulled out of the published `harness` plugin on 2026-09-12, pending fixes.**
Nothing in `incubator/` is installable — the marketplace only exposes `plugins/`. The code
runs; it is here because a cost-control tool that has not itself been measured has no
business being installed by anyone else.

## Why it was pulled — measured, not guessed

Verified against 246 local transcripts and live `claude -p` calls:

**A confirmed bug.** `context-guard.mjs` ranks warnings `{soft:1, loop:2, hard:3}` and
only fires when the new rank beats the stored one. But `loop` is a different *kind* of
warning, not a higher severity — so a loop warning at 5% context permanently suppresses
the 65% context warning for the rest of the session. Only the 80% warning survives.
Reproduced: same session, 5% + loop signal warns; the same session at 70% goes silent,
while a fresh session at 70% warns normally. The loop detector fires in 20/246 (8%) of
real sessions, so this is live, not theoretical.

**The drift layer costs more than it returns.** One drift check, invoked exactly as the
hook invokes it, bills **26,489 input tokens / $0.0284**. Adding
`--strict-mcp-config --setting-sources ""` gives the identical answer for
**21,380 tokens / $0.0105** — 63% cheaper. Across local history, 631 prompts landed in the
`[40%, 65%)` band → ~157 calls → **~$4.46**, corroborated by surviving state files
(`semCount` 450, 96, 28 ≈ 143 calls). Of the three, one session made ~112 calls
(~$3.18) with `driftWarned: false` — zero warnings produced.

**The band is mistimed on a 1M window.** `[40%, 65%)` of 1M is 400k–650k tokens. The
advice to start fresh arrives long after the history has been paid for on every turn.
Gate on absolute tokens, not a fraction.

**The "original goal" is wrong ~10% of the time.** `firstUserText` returns the first user
text block, which in 25/246 sessions is `<local-command-caveat>Caveat: The messages below
were generated by the user while running…`. Those sessions grade drift against harness
boilerplate.

**Accurate enough, for the record:** fill detection reads `usage` correctly and prefers
the authoritative window from `.ctx.json`; `session-weight`'s ~4 chars/token estimate came
in 13% low on a real technical README (3,667 actual vs 3,200 estimated, 3.49 chars/token),
which errs toward under-warning and matches its own "consistently rough" claim.

## Before this ships again

1. Split the loop warning out of the fill ladder — separate state key.
2. Add `--strict-mcp-config --setting-sources ""` to the `claude` provider argv, and pass
   `SESSION_WEIGHT_OFF: "1"` in the subprocess env alongside the existing
   `CONTEXT_GUARD_OFF`, since the nested `claude -p` fires `SessionStart` hooks too.
3. Re-tune the drift band to absolute tokens.
4. Skip `<local-command-caveat>` / `<command-name>` blocks in `firstUserText`.
5. Add a regression test for 1 and 4 before re-publishing.

## Wiring it by hand

`hooks/hooks.json` is a plugin manifest and does nothing from `incubator/`. To run these
while they live here, point at them by absolute path in `~/.claude/settings.json` — and
note the status line is already wired that way:

```json
"statusLine": {
  "type": "command",
  "command": "python3 /path/to/llm-toolbox/incubator/harness-sensors/statusline/statusline.py",
  "padding": 0
}
```
