...
carries the measurements and the fix list. It also holds **`mirror-test/`**, a skill
that tests whether a model's self-claims (its stated confidence, "I'd catch that", how
stable its answers are) match its behaviour, and can compare several models side by
side. You can use it now by symlinking it into `~/.claude/skills/`.
